...
def sampling_v2(task, x, y, diffusion, guidance_scale: float):
    nds = NonDominatedSorting()
    fronts, ranks = nds.do(y, return_rank=True)
    ...
    for i, rank_i in enumerate(selected_ranks):
        ...
        if mask.sum() == 0:
            ...
            continue
        print(f"{rank_i} -> {mask.sum()}")
        cond_points = y[mask]
        res_x = diffusion.sample(
            batch_size=mask.sum(),
            cond=torch.from_numpy(cond_points),
        ...
        all_points[f"res_x_{rank_i}"] = res_x
        all_points[f"res_y_{rank_i}"] = res_y
        all_points[f"cond_points_{rank_i}"] = cond_points
    return all_points
...
def main():
    ...
    # Sample the model at using different data points IN the dataset
    all_points = sampling_v2(
        task, X, y, diffusion=ema_model, guidance_scale=config.guidance_scale
    )
